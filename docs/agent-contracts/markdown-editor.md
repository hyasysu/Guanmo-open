...
- 预览选区由组件接管（渲染时注入 `<span data-gm-src-from/to>` 源码标注）：拖选经 rAF 节流映射为源码 offset，Ctrl+C 走 `getTextForSourceRange` 而非 DOM Selection；KaTeX / 代码高亮等无精确 position 的子树降级为原生行为。禁止退回"空闲后逐渐渲染全文 DOM"或 DOM 文本长度推测源码位置。
