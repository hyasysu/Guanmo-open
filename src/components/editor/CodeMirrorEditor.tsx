...
function splitFontFamilyList(value: unknown): string[] {
...
function composeEditorFontFamily(fontFamily: unknown, previewFontFamily: unknown): string {
  const editorTokens = splitFontFamilyList(fontFamily)
  const previewTokens = splitFontFamilyList(previewFontFamily)
  if (editorTokens.length === 0) return previewTokens.join(', ') || 'monospace'

  const seen = new Set(editorTokens.map(normalizeFontToken))
  const uniquePreviewTokens = previewTokens.filter((token) => {
    const normalized = normalizeFontToken(token)
    if (!normalized || seen.has(normalized)) return false
    seen.add(normalized)
    return true
  })
  if (uniquePreviewTokens.length === 0) return editorTokens.join(', ')

  const genericIndex = editorTokens.findIndex(isGenericFontFamily)
  if (genericIndex < 0) {
    return [...editorTokens, ...uniquePreviewTokens].join(', ')
  }
...
export function CodeMirrorEditor({ content, onChange, onSave, onImageFiles, viewRef: externalViewRef, documentKey, tabId, initialScrollTop, initialCursor, initialSelection, initialRanges, initialMainIndex, onBeforeDestroy, resource = 'editor' }: CodeMirrorEditorProps) {
  ...
  const guanmoTheme = useMemo(
    () => buildMarkdownEditorTheme(editorSettings.fontSize, editorSettings.lineHeight, effectiveFontFamily),
