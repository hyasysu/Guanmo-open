...
/**
 * Generate AI migration prompt for user to copy.
 */
export function generateMigrationPrompt(
  sqliteCounts: LegacyStoreCounts,
  legacyCounts: LegacyStoreCounts,
): string {
  const sqlitePath = getSqliteDatabasePath()
  const indexedDBPath = getLegacyIndexedDBPath()

  return `我需要将旧版观墨（IndexedDB）的数据迁移到新版（SQLite）。

数据库绝对路径：
- SQLite（新版）：${sqlitePath}
- IndexedDB（旧版）：${indexedDBPath}

旧版 IndexedDB 数据统计：
- 文档：${legacyCounts.documents}
- 会话：${legacyCounts.chat_sessions}
- 消息：${legacyCounts.chat_messages}
- 记忆：${legacyCounts.memories}

新版 SQLite 数据统计：
- 文档：${sqliteCounts.documents}
- 会话：${sqliteCounts.chat_sessions}
- 消息：${sqliteCounts.chat_messages}
- 记忆：${sqliteCounts.memories}
