...
async function run() {
  ...
  const restoreTabsIndex = app.indexOf('await restoreTabs()', databaseInitIndex)
  const appReadyIndex = app.indexOf('setAppReady(true)', restoreTabsIndex)
  const warmupIndex = app.indexOf('scheduleIdleWarmup()', appReadyIndex)
  assert.ok(databaseInitIndex >= 0 && restoreTabsIndex > databaseInitIndex)
  assert.ok(appReadyIndex > restoreTabsIndex && warmupIndex > appReadyIndex)
