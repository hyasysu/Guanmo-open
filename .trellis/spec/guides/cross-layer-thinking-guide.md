...
## Persisted UI State vs Native Capability State

Persisting a filesystem path in frontend state does not persist the native capability that made the original access legal. After an application restart, the path may still exist while the backend authorization set is empty.

When tightening a native permission boundary:

- Inventory recent items, favorites, restored tabs, history/source links, restored workspaces, background indexes, and every other persisted-path consumer.
- Persist grants only from trusted native boundaries such as system dialogs, file associations, or native drag-drop events; never rebuild grants directly from frontend storage or database paths.
- Restore valid canonical grants before frontend session hydration.
- Give legacy entries a one-time same-path native reauthorization flow, and prompt only for an explicit authorization error—not for missing files, invalid extensions, or OS access errors.
- Add a regression audit that forbids persisted-path consumers from calling the fresh-selection authorization API.
- Preserve temporarily unavailable trusted grants and one-time migration candidates in a bounded backend-owned retry queue; never let an offline drive turn grant restoration into permanent data loss.
- Treat filesystem metadata calls on removable, network, or UNC paths as blocking I/O: move them off startup/IPC threads, serialize state mutations in the backend, and preserve authorization-before-read ordering in the frontend.

**Real-world example**: A file-policy hardening correctly rejected arbitrary frontend authorization, but recent files, favorites, restored tabs, historical AI sources, and restored workspaces still replayed persisted paths as fresh dialog selections. Unit policy tests passed while restart behavior broke. The durable fix was a Rust-owned grant store, same-path legacy recovery, and a static call-site audit.
