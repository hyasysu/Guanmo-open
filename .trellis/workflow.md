...
1. **Explicit opt-in only** — enter this workflow only when the current user message explicitly requests Trellis; prior consent does not carry forward
2. **Plan before code** — figure out what to do before you start
3. **Specs injected, not remembered** — guidelines are injected via an explicitly requested skill, not recalled from memory
4. **Persist everything** — research, decisions, and lessons all go to files; conversations get compacted, files don't
5. **Incremental development** — one task at a time
